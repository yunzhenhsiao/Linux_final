...
**表 3：`databases/relational/schema.sql` 定義了 10 張資料表，涵蓋系統的完整業務實體**
| 資料表                                       | 說明                          | 關鍵欄位                                                              |
| -------------------------------------------- | ----------------------------- | --------------------------------------------------------------------- |
...
**表 5：- **確定性 Fallback 機制**：小型模型（1B 參數）在複雜查詢上工具選擇準確率較低，系統額外實作了基於關鍵字的確定性路由規則，覆蓋以下常見查詢類型**
| 觸發關鍵字                                           | 選擇的工具                  |
| ---------------------------------------------------- | --------------------------- |
...
**表 15：為求客觀驗證，本專案撰寫 `benchmark_cache.py` 實際針對 Docker 環境中的 PostgreSQL 與 Redis 進行各 50 次反覆讀取壓力測試，實測結果如下**
